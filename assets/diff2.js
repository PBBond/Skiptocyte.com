...
function myFunction(e) {
  $(e.target.nextElementSibling).slideToggle(250);
...
document.addEventListener("click", function (e) {
  if (
    e.target.classList[0] === "dropdown-link" &&
    e.target.children[0].nodeName == "INPUT"
  ) {
    //nothing happens, dropdown closes
  } else if (e.target.classList[0] === "dropdown-link") {
    if (
      e.target.nodeName != "I" &&
      !e.target.classList.contains("dd-trash-btn")
    ) {
...
document
  .querySelector(".max-wbc-input")
  .addEventListener("keydown", enterNumbers);
...
function updateRelative(i) {
  for (i = 0; i < counttd.length; i++) {
    let relative =
      (parseInt(counttd[i].innerHTML) / parseInt(currentCount.innerHTML)) * 100;
    if (relative) {
...
function absoluteCounter(e) {
  ...
  for (i = 0; i < absolutetd.length; i++) {
    ...
    if (absolute) {
      absolutetd[i].textContent = absolute.toFixed(
        intsAfterDec(wbcInput[0].value)
      );
    } else {
      absolutetd[i].textContent = "";
    }
      ...
      if (e.key === keytd[i].value) {
        ...
        //add, if the cell is empty add a 0
          ...
          if (isNaN(denominator) || numerator < denominator) {
            ...
            addCounter(i), updateRelative(i), absoluteCounter(e);
            if (
              Number(currentCount.innerText) >=
              Number(maxWbcCountDenominator.innerText)
            ) {
              ding.play();
            }
          } else {
...
function savePresetToList() {
  ...
  let keyCellsTemp = [];
  for (i = 0; i < keyInput.length; i++) {
    let temp = [keyInput[i].value, cellInput[i].value];
...
function checkMark() {
  ...
  setTimeout(function () {
    updateBtn[0].innerHTML =
      '<i class="fas fa-save"></i><span>Update Preset</span>';
  }, 2500);
}
